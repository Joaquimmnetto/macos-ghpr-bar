...
import (
	...
	"github.com/joaquimmnetto/gh-bar-macos/view"
	"os"
	"os/exec"
	"path/filepath"
	"regexp"
...
func main() {
	...
	config, err := core.LoadConfiguration(configurationFile)
	if err != nil {
		native.FNSLog("%e", err)
		native.FNSLog("Loading configuration file at %s", defaultConfigurationFile)
		config, err = core.LoadConfiguration(defaultConfigurationFile)
		if err != nil {
			native.FNSLog("%e", err)
...
func setupStatusBar(app appkit.Application, config core.Configuration) {
...
func renderStatusMenu(app appkit.Application, statusItem appkit.StatusItem, menu appkit.Menu, prs []github.PullRequest, config core.Configuration) {
...
func renderPRs(menu appkit.Menu, allPRs []github.PullRequest, config core.Configuration) int {
